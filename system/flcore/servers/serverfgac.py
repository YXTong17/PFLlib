...
import torch
from flcore.clients.clientfgac import clientFGAC
...
class FedFGAC(Server):
    ...
    def aggregate_parameters(self):
        ...
        for w, client_model, classifier_weight in zip(
            self.uploaded_weights, self.uploaded_models, classifier_weights
        ):
            # 聚合除最后一层全连接层之外的所有层
            for server_param, client_param in zip(
                self.global_model.base.parameters(), client_model.base.parameters()
            ):
                server_param.data += client_param.data.clone() * w
            # 聚合最后一层全连接层
            for server_param, client_param in zip(
                self.global_model.head.parameters(), client_model.head.parameters()
            ):
                for class_idx, c_w in enumerate(classifier_weight):
                    server_param.data[class_idx] += client_param.data[class_idx] * c_w
